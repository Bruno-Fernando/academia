...
public class IpirangaImpostoSimplesBRTest {
	...
	private String ppJrTaxista = "222.222.222-22";
	...
	private void testeNomesEBens() throws Exception {
		...
		Assert.assertEquals(0.01, ipiranga.getBens(ppTaxista), 0.001);
		ipiranga.adicionarBem(rrCaminhoneiro, 9.99);
		Assert.assertEquals(9.99, ipiranga.getBens(rrCaminhoneiro), 0.001);
		for (int i = 0; i < 10; i++) {
			ipiranga.adicionarBem(ffProfessor, 0.01);
		}
		Assert.assertEquals(0.1, ipiranga.getBens(ffProfessor), 0.001);
	}
